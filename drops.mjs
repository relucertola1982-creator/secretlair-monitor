...
export default async (req) => {
  ...
  if (!refresh) {
    try {
      const cached = await store.get('current-products');
  ...
  if (!CLAUDE_KEY) {
    return new Response(JSON.stringify({ error: 'CLAUDE_KEY not set' }), { status: 500, headers: HEADERS });
  }

  try {
    const drops = await checkDropsWithClaude(CLAUDE_KEY);
    const now = new Date().toISOString();
    await store.set('current-products', JSON.stringify(drops));
